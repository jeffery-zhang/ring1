...
- [ ] A2. 确认非 TTY 缺少参数时策略(回退默认值 or 直接报错)
- [ ] A3. 固化交互文案与默认选中项
...
- [ ] D3. 实现 mode 单选交互(link/copy)
- [ ] D4. 实现非交互环境兜底分支

## 模块 E: CLI 编排接入

- [ ] E1. 在 `sync` 命令接入“参数优先，缺参交互”流程
- [ ] E2. 保持原有参数校验、错误输出与退出码语义
- [ ] E3. 验证三目标执行汇总与 Windows 自动降级逻辑

## 模块 F: 验证与文档收尾

- [ ] F1. 验证无参数启动的交互流程(agents + mode)
- [ ] F2. 验证带参数启动时不进入交互流程
- [ ] F3. 验证 opencode 路径写入与备份行为
- [ ] F4. 更新 README 中支持列表、路径说明与示例
- [ ] F5. 同步更新实施计划勾选状态并标记完成
- [ ] F6. 若验证出现问题，记录到 `memory/validations`
